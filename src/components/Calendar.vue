...
export default {
  ...
  methods: {
    ...
    async getEvents() {
      ...
      let events = []
      // Loop through documents, append id to each data object, push each to events array
    ...
    // update edited event to Firebase
    ...
    // Change calendar view to Day
    viewDay({ date }) {
      this.focus = date
      this.type = 'day'
    },
    // Get color for target event
    getEventColor(event) {
      return event.color
    },
    ...
    // Previous and next functions to cycle through month
    prev() {
    ...
    // Function to show event on click
